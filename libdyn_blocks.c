 ...
 // printf("comp_func bilint: flag==%d\n", flag);
  ...
  switch (flag) {
    ...
    case COMPF_FLAG_CONFIGURE:  // configure
      ...
      T_a = 1;
      Qn[0] = T_a/2; Qn[1] = T_a/2; Qd[0] = -1; Qd[1] = 1; // Bilineare Trafo
 ...
 // printf("comp_func TP1: flag==%d\n", flag);
  ...
  switch (flag) {
    ...
    case COMPF_FLAG_CONFIGURE:  // configure
      ...
      Qn[0] = 1-z_oo; Qn[1] = 0; Qd[0] = -z_oo; Qd[1] = 0; // 1-z_oo / z - z_oo
...
int compu_func_delay(int flag, struct dynlib_block_t *block)
{
  ...
  int *buf_position = &( (int*) buffer__)[0];
  double *rot_buf = (double*) &( (int*) buffer__)[1];
  ...
  switch (flag) {
    case COMPF_FLAG_CALCOUTPUTS:
      ...
      *out = rot_buf[*buf_position];
      rot_buf[*buf_position] = *inp;
      printf("out %f, in %f, pos %d\n", *out, *inp, *buf_position);
      return 0;
      break;
    case COMPF_FLAG_UPDATESTATES:
      if (*buf_position >= delay_length) {
	*buf_position = 0;
      } else {
	*buf_position++;
      }
    ...
    {
      void *buffer = malloc(sizeof(double)*delay_length + sizeof(int));
