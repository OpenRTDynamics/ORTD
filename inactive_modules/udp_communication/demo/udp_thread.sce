...
thispath = get_absolute_file_path('udp_thread.sce');
cd(thispath);
...
function [sim, outlist] = run_thread_fn(sim, inlist)
  ...
  //
  [sim] = ld_printf(sim, defaultevents, out, "Received something from the UDP-Socket", 2);
...
save_irparam(par, 'udp_thread.ipar', 'udp_thread.rpar');
