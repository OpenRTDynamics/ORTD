...
function [sim,bid] = libdyn_new_blk_const(sim, events, c1)
  btype = 40;
  [sim,bid] = libdyn_new_blk_generic(sim, events, btype, [], [c1]);
endfunction

function [sim,bid] = libdyn_new_blk_gain(sim, events, c)
  btype = 20;
  [sim,bid] = libdyn_new_blk_generic(sim, events, btype, [], [c]);
endfunction
...
function sim = libdyn_connect_block(sim, src, src_port, dst, dst_port)
  ...
  sim.cllist($+1) = [0, src_id, src_id, src_port,   0, dst_id, dst_id, dst_port];
endfunction
