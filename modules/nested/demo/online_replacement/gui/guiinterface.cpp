    ...
    ui(new Ui::GuiInterface)
{
    ...
    qt_event_com->scope_ref_data = new QPL_ScopeData(1,0.01,tr("1"),QStringList("1") );  // scope with two traces; add more << "n" as you increase traces
...
void GuiInterface::on_pushButton_pressed()
{
    char tmp[200];

    if (ReloadState == 1) {
      sprintf(tmp, "set_param gui_trigger_reload # %d \n", 0);
      ReloadState = 0;
    } else {
        sprintf(tmp, "set_param gui_trigger_reload # %d \n", 1);
        ReloadState = 1;
